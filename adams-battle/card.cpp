...
void Tank::attack(vector<Card*> &field,int round) // normal saldiri. Tanklarin özelligi diger kartlarýn saldirisini üzerine cekmektir
{

    // skill aticak cd geldiyse
    this->autoattack(field) ;
    this->hasAttacked = true ;

}
void Marksman::attack(vector<Card*> &field,int round) // marksman kritik vurma ihtimali vardir
{
...
void Assasin::attack(vector<Card*> &field,int round) // assasin cani en az olana vurur
{
...
void Mage::attack(vector<Card*> &field,int round) // Mage oyun ilerledikce güclenir her attack yaptiginda atk 5 artar
{
...
void Support::attack(vector<Card*> &field,int round) // support attack yaptiktan sonra can basmali
{
...
void Tank::autoattack(vector<Card*> &field) // normal sekilde ilk siradakine vurur
{
    ...
    else
    {
       ...
       field[0]->setHp(newHp) ;
       cout << this->name << " attacked " << field[0]->name << " for " << this->getAtk() << "x" << compareElements(field[0])  << "=" << this->getAtk()*compareElements(field[tankindex]) << " damage. " << field[0]->name << "'s remaining hp: " << field[0]->getHp() << "\n" ;
    }
}

void Assasin::autoattack(vector<Card*> &field) // caný az olana vurur
{
    int enazcanindex = 0;
...
void Marksman::autoattack(vector<Card*> &field) // kritik vurma ihtimali vardir
{
    ...
    else
    {
       ...
       field[0]->setHp(newHp) ;
       cout << this->name << " attacked " << field[0]->name << " for " << this->getAtk() << "x" << compareElements(field[0]) << "x" << extraDmg << "=" << this->getAtk()*compareElements(field[tankindex])*extraDmg << " damage. " << field[0]->name << "'s remaining hp: " << field[0]->getHp() << "\n" ;
    }
